...
def mkExcel(queryset, folder_name):
    ...
    for obj in queryset:
        ...
        # 社員番号
        sheet.cell(data_row, dataMst[6][1], aggEmpNo)
        # 社員名前
        sheet.cell(data_row, dataMst[7][1], aggName)
        # 実働時間
        sheet.cell(data_row, dataMst[8][1], aggWorkTime)
        # 出勤日数
        sheet.cell(data_row, dataMst[9][1], aggAttendCount)
        # 欠勤日数
        sheet.cell(data_row, dataMst[10][1], aggAbsCount)
        # 年休日数
        sheet.cell(data_row, dataMst[11][1], aggAnnLeave)
        # 休出日数
        sheet.cell(data_row, dataMst[12][1], aggRestCount)
        # 遅早退日数
        sheet.cell(data_row, dataMst[13][1], aggLateCount)
        # 統計年月
        sheet.cell(headMst[6][0], headMst[6][1], str(objMonth))
        # 行ごとに書式のコピー
        i = i + 1
        for a in range(1, 9):
